...
describe('Testing metadata', () => {
  ...
  it('should create a new "jeux de données"', () => {
    ...
    cy.get(
      '[data-qa="pnx-dataset-form"] > div.row > div.col-md-4 > div:nth-child(1) > div.card-body > pnx-metadata-actor > div > form > ng-select > div > div > div.ng-input'
    ).click();
    cy.get('[data-qa="pnx-metadata-organism-ALL"]').click();
    cy.get("[data-qa='pnx-dataset-form-save-jdd'] ").should('be.disabled');
  ...
  it('should delete the new "jeux de données"', async () => {
    ...
    const id =
      myCadreAcq[0].parentElement.childNodes[1].firstChild.firstChild.childNodes[1].childNodes[0]
        .childNodes[0].innerText;
    cy.get('[data-qa="pnx-metadata-jdd-nb-obs-' + id + '"]').contains('0');
    cy.get('[data-qa="pnx-metadata-jdd-delete-' + id + '"]').click({ force: true });
  ...
  it('should display data of the "cadre d\'acquisition"', async () => {
    ...
    const listData = await promisify(
      cy.get(
        "[data-qa='pnx-synthese-list'] > ngx-datatable > div > datatable-body > datatable-selection > datatable-scroller"
      )
    );
    // too much aleatory, not work on some computer..
