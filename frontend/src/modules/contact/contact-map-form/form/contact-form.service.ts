...
export class ContactFormService {
  ...
  getDefaultValues(idOrg?: number, regne?: string, group2_inpn?: string) {
    ...
    return this._http.get<any>(`${AppConfig.API_ENDPOINT}/contact/defaultNomenclatures`, {
      params: params
    });
  }
  ...
  initReleveForm(): FormGroup {
    const releveForm = this._fb.group({
      ...
      properties: this._fb.group({
        ...
        observers: [null, !ContactConfig.observers_txt ? Validators.required : null],
  ...
  onTaxonChanged(taxon) {
    ...
    this.getDefaultValues(this.currentUser.organismId, taxon.regne, taxon.group2_inpn).subscribe(
      data => {
        // occurrence
        this.patchDefaultNomenclatureOccurrence(data);
        // counting
