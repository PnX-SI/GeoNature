...
import { LangChangeEvent, TranslateService } from '@librairies/@ngx-translate/core';
import { Subject } from '@librairies/rxjs';
import { takeUntil } from '@librairies/rxjs/operators';
...
export class SyntheseCriteriaService implements OnDestroy {
  ...
  private prepareSeveralValuesEntry() {
    ...
    if (severalValuesEntry) {
      if (!severalValuesEntry.hasOwnProperty('label') || severalValuesEntry.label == '') {
  ...
  private prepareUnknownValueEntry() {
    ...
    if (unknownValueEntry) {
      if (!unknownValueEntry.hasOwnProperty('label') || unknownValueEntry.label == '') {
        unknownValueEntry.label = this.translateService.instant('Synthese.Map.UnknownValue');
      }
    } else {
      this.selectedCriteria.values.push({
        value: '?',
        label: this.translateService.instant('Synthese.Map.UnknownValue'),
        color: '#EC4300', // Orange
  ...
  getCriteriaStyle(observations) {
    ...
    for (let value of values) {
      if (type == 'nomenclatures') {
        ...
        if (distinctCriteriaStyles.has(criteriaStyle.color) === false) {
