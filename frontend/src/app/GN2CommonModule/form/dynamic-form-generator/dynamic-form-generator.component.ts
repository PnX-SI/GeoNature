...
export class GenericFormGeneratorComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes) {
    ...
    // Process changed
    if (changes.defaults && !changes.defaults.firstChange) {
      const defaults = changes.defaults.currentValue;
      const defaultKeys = new Set(Object.keys(defaults));

      // Clean existing controls if necessary (not autogenerated, and not in form.defaults)
      if (!this.autoGenerated) {
        const formsToRemove = this.formsSelected.filter(
          (formDef) => !defaultKeys.has(formDef.attribut_name)
        );
        formsToRemove.forEach((formDef) => {
          const index = this.formsSelected.indexOf(formDef);
          if (index !== -1) {
      ...
      for (const [defaultKey, defaultValue] of Object.entries(defaults)) {
        ...
        // Add missing control
        if (!this.myFormGroup?.contains(defaultKey)) {
          const formDef = this.formsDefinition.find(
            (definition) => definition.attribut_name === defaultKey
          );
          if (formDef) {
