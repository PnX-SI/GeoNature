...
import { PublicationFormModalComponent } from '../publication-form-modal/publication-form-modal.component';
...
export class PublicationCardComponent implements OnInit {
  ...
  loadPublication(): void {
    ...
    this.publicationsListService.getPublication(this.id_publication).subscribe(
      ...
      (err) => {
        ...
        if (err?.status === 404) {
          this._commonService.translateToaster('error', 'MetaData.Publications.Errors.NotFound');
        }
  ...
  getPublicationTypeLabel(idNomenclature: number): string {
    ...
    return type?.label_default ?? (idNomenclature ? String(idNomenclature) : '-');
  }
}
