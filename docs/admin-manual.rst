...
Les champs additionnels d'Occtax permettent de démultiplier les possibilités du formulaire. Lorsqu'on associe des champs additionnels à un jeu de données, on souhaite parfois dissocier le module Occtax avec les champs habituels du standards du module "Occtax" que l'on a augmenté avec des champs additionnels.
Il possible d'ajouter une entrée dans la table `gn_commons.t_modules` pour simuler un "nouveau module" dans la liste latérale des modules GeoNature. En passant les paramètres `id_dataset` et `module_label` à la colonne `module_path`, on est alors redirigé vers le module "Occtax" préfiltré uniquement avec les données de notre JDD. Le nom du module dans la barre superieure est également changé. Le JDD sera également pré-sélectionné dans le formulaire et les champs additionnels automatiquements ajoutés.
