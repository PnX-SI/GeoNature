...
CREATE FUNCTION taxonomie.check_is_group2inpn(mygroup text) RETURNS boolean
  ...
  END;
$$;
CREATE FUNCTION taxonomie.check_is_group3inpn(mygroup text) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
...
ALTER FUNCTION taxonomie.check_is_group3inpn(mygroup text) OWNER TO geonatadmin;
