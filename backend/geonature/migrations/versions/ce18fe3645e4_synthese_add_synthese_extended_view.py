"""[synthese] Add synthese_extended view

Revision ID: ce18fe3645e4
Revises: 707390c722fe
Create Date: 2025-06-03 19:12:03.634042
...
def upgrade():
    ...
    op.execute(
        """
        CREATE VIEW gn_synthese.synthese_extended AS
            ...
            FROM gn_synthese.synthese AS s
                ...
                LEFT JOIN gn_synthese.t_sources sources
...
def downgrade():
    op.execute(
        """
        DROP VIEW gn_synthese.synthese_extended;
        """
    )
        ...
        CREATE OR REPLACE VIEW gn_synthese.v_synthese_for_web_app AS
            ...
            FROM gn_synthese.synthese AS s
                ...
                JOIN gn_meta.t_datasets AS d
                    ON d.id_dataset = s.id_dataset
                JOIN gn_synthese.t_sources sources
                    ON sources.id_source = s.id_source ;
        """
    )
