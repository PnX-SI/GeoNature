"""[synthese] add biogeo status

Revision ID: cb663f039774
Revises: a90fa0630ef5
Create Date: 2026-09-04 15:21:05.413864
...
revision = "cb663f039774"
down_revision = "a90fa0630ef5"
branch_labels = None
depends_on = None
...
def downgrade():
    logger.warning(
        "La colonne id_nomenclature_biogeo_status ne peut pas être supprimée de la vue "
        "gn_synthese.v_synthese_for_web_app sans la recréer (DROP + CREATE), ce qui pourrait casser "
    ...
    op.execute(
        ...
        SELECT s.id_synthese,
            ...
            t.famille,
            t.sous_famille,
            NULL::integer AS id_nomenclature_biogeo_status
        FROM gn_synthese.synthese s
            JOIN taxonomie.taxref t ON t.cd_nom = s.cd_nom
            JOIN gn_meta.t_datasets d ON d.id_dataset = s.id_dataset
            JOIN gn_synthese.t_sources sources ON sources.id_source = s.id_source;
