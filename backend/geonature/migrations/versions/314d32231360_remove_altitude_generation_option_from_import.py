...
def upgrade():
    ...
    op.execute(
        """
        create trigger tri_synthese_calculate_alt before
        insert
        on
        gn_synthese.synthese for each row execute function ref_geo.fct_trg_calculate_alt_minmax('the_geom_4326')
        """
    )
...
def downgrade():
    ...
    op.execute(
        """
        drop trigger if exists tri_synthese_calculate_alt on gn_synthese.synthese;
        """
    )
