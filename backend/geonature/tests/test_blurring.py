...
@pytest.fixture(scope='class')
def blurred_users():
    from geonature.core.gn_permissions.models import TActions, BibFiltersType, \
...
def sensitivity_rule(synthese_data):
    ...
    department_diffusion = TNomenclatures.query.filter_by(id_type=sensitivity_nomenc_type.id_type,
                                                          mnemonique='3').one()
    s = synthese_data[0]
    with db.session.begin_nested():
        rule = SensitivityRule(cd_nom=s.cd_nom,
