...
from geonature.core.gn_profiles.models import (
    ...
    TParameters, CorTaxonParameters
)
from geonature.core.gn_synthese.models import Synthese
from geonature.core.taxonomie.models import Taxref


from .fixtures import (acquisition_frameworks, 
                       datasets,
                       synthese_data,
                       source)
...
def get_gn_profile_data():
    ...
    id_rang = TParameters.query.with_entities(TParameters.name, TParameters.value).filter_by(name="id_rang_for_profiles").one()
    id_rang = id_rang.value.split(',')[0]

    cd_nom = Taxref.query.with_entities(Taxref.cd_nom, Taxref.id_rang).filter_by(id_rang=id_rang).first().cd_nom
