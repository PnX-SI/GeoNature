...
class TestRefGeo:
    ...
    def test_get_area_intersection(self, area_commune):
        ...
        communes = { area['area_name'] for area in response.json[str(area_commune.id_type)]['areas'] }
        assert communes == self.expected_communes
    ...
    def test_get_areas_enable_wrong(self):
        response = self.client.get(url_for("ref_geo.get_areas"),
                                   query_string={"enable": "wrong"})
        
        assert response.status_code == 400
        assert response.json["message"] == "Le paramètre 'enable' accepte seulement les valeurs: true, false, all."
    ...
    def test_get_areas_id_type(self, area_commune):
        ...
        response = self.client.get(url_for("ref_geo.get_areas"),
                                   query_string={"id_type": id_type_commune})
    ...
    def test_get_areas_type_code(self, area_commune):
        ...
        response = self.client.get(url_for("ref_geo.get_areas"),
                                   query_string={"type_code": type_code})
