...
class TestChecks:
    ...
    def test_check_required_values_geom_conditions(self, imprt):
        fields = get_fields(imprt, ["WKT", "latitude", "longitude", "codecommune", "codedepartement", "codemaille"])
        assert fields["WKT"].mandatory is True
        assert set(fields["WKT"].optional_conditions) == {"latitude", "longitude", "codecommune", "codedepartement", "codemaille"}
        assert fields["WKT"].mandatory_conditions is None
        assert fields["latitude"].mandatory is True
        assert set(fields["latitude"].optional_conditions) == {"WKT", "codecommune", "codedepartement", "codemaille"}
        assert set(fields["latitude"].mandatory_conditions) == {"longitude"}
        assert fields["longitude"].mandatory is True
        assert set(fields["longitude"].optional_conditions) == {"WKT", "codecommune", "codedepartement", "codemaille"}
        ...
        assert_errors(
            ...
            expected=[
                ...
                Error(
                    error_code="MISSING_VALUE", column="latitude", invalid_rows=frozenset([2])
                ),
                Error(
                    error_code="MISSING_VALUE", column="longitude", invalid_rows=frozenset([1])
                ),
            ],
        )
