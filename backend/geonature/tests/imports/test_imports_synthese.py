...
class TestImportsSynthese:
    ...
    def test_import_values(self, users, loaded_import):
        ...
        schema = {
            ...
            "type": "object",
            "patternProperties": {
                "^.*$": {  # keys are nomenclature mnemoniques
                    "type": "object",
                    "properties": {
        ...
        self.assert_import_values_by_nomenclature(r.json)

    def assert_import_values_by_nomenclature(self, import_values):
        """Assert that each nomenclature type is returned only once, keyed by its mnemonique."""
        known_mnemoniques = set()
        for key, value in import_values.items():
            # response is keyed by the nomenclature mnemonique
            assert key == value["nomenclature_type"]["mnemonique"], (
                key,
                value["nomenclature_type"]["mnemonique"],
            )
            # each nomenclature type is only returned once
            assert value["nomenclature_type"]["mnemonique"] not in known_mnemoniques
            known_mnemoniques.add(value["nomenclature_type"]["mnemonique"])
            assert isinstance(value["values"], list)
            for item in value["values"]:
                assert set(item.keys()) == {"value", "mandatory"}
                assert isinstance(item["mandatory"], bool)

    def test_import_values_aggregate_nomenclatures(self, users, loaded_import):
        """
        Nomenclatures shared by several fields are returned once with the
        aggregation of the source values of all the fields using them, each
        value being flagged mandatory if used by a mandatory field.
        """
        imprt = loaded_import
        life_stage = db.session.scalar(
            sa.select(BibFields).where(
                BibFields.destination == imprt.destination,
                BibFields.name_field == "id_nomenclature_life_stage",
            )
        )
        assert life_stage is not None
        assert life_stage.mnemonique == "STADE_VIE"
        with db.session.begin_nested():
            # A second field sharing the STADE_VIE nomenclature, mapped to "sexe".
            # It is the mandatory field of the scenario: the values coming only
            # from it must be flagged as mandatory.
            shared_field = BibFields(
                id_destination=imprt.destination.id_destination,
                name_field="id_stade_vie_extra",
                source_field="src_id_nomenclature_sex",
                dest_field="id_stade_vie_extra",
                fr_label="Stade de vie (extra)",
                eng_label=None,
                type_field="integer",
                mandatory=True,
                autogenerated=False,
                mnemonique=life_stage.mnemonique,
                display=True,
                multi=False,
            )
            db.session.add(shared_field)
            imprt.fieldmapping[shared_field.name_field] = {"column_src": "sexe"}
            db.session.flush()
        ...
        # STADE_VIE is returned only once, with the values of both fields
        assert "STADE_VIE" in r.json
        stade_vie_values = {
            item["value"]: item["mandatory"] for item in r.json["STADE_VIE"]["values"]
        }
        # value coming only from the mandatory extra field is mandatory
        assert "Femelle" in stade_vie_values
        assert stade_vie_values["Femelle"] is True
        # values coming only from the optional life stage field are not mandatory
        assert {"Adulte", "Immature", "Juvénile"} <= set(stade_vie_values)
        assert stade_vie_values["Adulte"] is False
        assert stade_vie_values["Immature"] is False
        assert stade_vie_values["Juvénile"] is False
        # no fake nomenclature is injected anymore
        assert all(n["cd_nomenclature"] is not None for n in r.json["STADE_VIE"]["nomenclatures"])
