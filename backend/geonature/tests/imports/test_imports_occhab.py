...
@pytest.fixture()
def fieldmapping(occhab_destination, autogenerate_fields):
    fields = BibFields.query.filter_by(destination=occhab_destination, display=True).all()
    return {
        field.name_field: field.name_field in autogenerate_fields
        if field.autogenerated
        else field.name_field
        for field in fields
    }
...
class TestImportsOcchab:
    ...
    @pytest.mark.parametrize("autogenerate_fields", [("unique_id_sinp_generate",)])
