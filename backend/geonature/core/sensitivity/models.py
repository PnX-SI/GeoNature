...
cor_sensitivity_area = db.Table("cor_sensitivity_area",
    db.Column("id_sensitivity",
              db.Integer,
...
class CorSensitivityCriteria(db.Model):
    ...
    id_sensitivity_rule = db.Column('id_sensitivity',
                                    db.Integer,
                                    ForeignKey(SensitivityRule.id),
                                    primary_key=True)
    sensitivity_rule = relationship(SensitivityRule,
                                    backref=backref('sensitivity_criterias',
                                    cascade='all, delete-orphan'))

    id_criteria = db.Column('id_criteria',
                             db.Integer,
                             ForeignKey(TNomenclatures.id_nomenclature),
                             primary_key=True)
    criteria = relationship(TNomenclatures)

    id_nomenclature_type = db.Column("id_type_nomenclature",
                                     db.Integer,
                                     ForeignKey(BibNomenclaturesTypes.id_type))
