...
from sqlalchemy import DateTime, Integer, String, Text, Unicode
...
class VmCorTaxonPhenology(db.Model):
    ...
    cd_ref: Mapped[int] = mapped_column(Integer, primary_key=True)
    doy_min: Mapped[int] = mapped_column(Integer, primary_key=True)
    doy_max: Mapped[int] = mapped_column(Integer, primary_key=True)
...
class VmValidProfiles(db.Model):
    ...
    first_valid_data: Mapped[Optional[datetime]] = mapped_column(DateTime)
    last_valid_data: Mapped[Optional[datetime]] = mapped_column(DateTime)
...
class VConsistencyData(db.Model):
    ...
    id_synthese: Mapped[int] = mapped_column(
        Integer, ForeignKey(Synthese.id_synthese), primary_key=True
    )
    ...
    def score(cls):
        return (
            cls.valid_distribution.cast(Integer)
            + cls.valid_phenology.cast(Integer)
            + cls.valid_altitude.cast(Integer)
        )
...
class VSyntheseForProfiles(db.Model):
    ...
    nom_cite: Mapped[Optional[str]] = mapped_column(Unicode(length=1000))
    cd_ref: Mapped[Optional[int]]
    nom_valide: Mapped[Optional[str]] = mapped_column(Unicode(length=500))
    id_rang: Mapped[Optional[str]] = mapped_column(Unicode(length=10))
    date_min: Mapped[Optional[datetime]] = mapped_column(DateTime)
    date_max: Mapped[Optional[datetime]] = mapped_column(DateTime)
