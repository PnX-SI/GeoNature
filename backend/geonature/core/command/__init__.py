...
def import_cmd(dirname):
    try:
        ...
        module_cms = __import__("{}.backend.commands.geonature_cmd".format(dirname))
        ...
        Path(os.path.dirname(e.filename)).mkdir(parents=True, exist_ok=True)
        import_cmd(dirname)
...
for dirname in os.listdir(plugin_folder):
    cmd_file = os.path.join(plugin_folder, dirname, "backend", "commands", "geonature_cmd.py")
