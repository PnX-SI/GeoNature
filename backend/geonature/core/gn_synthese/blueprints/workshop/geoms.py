...
def _build_base_obs_subquery(permissions, filters):
    """
...
def _geom_area_mode(filters, permissions, area_aggregation_type):
    ...
    obs_subquery = _build_base_obs_subquery(permissions, filters)
    ...
    geojson_features = []
    for geom_as_geojson, id_area, observation_count in results.all():
        geojson_features.append(
            Feature(
                geometry=json.loads(geom_as_geojson) if geom_as_geojson else None,
                properties={"id_area": id_area, "observation_count": observation_count},
            )
        )
