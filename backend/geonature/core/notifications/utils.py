...
class Notification:
    ...
    def create_notification(requestData):
        ...
        for role in idRoles:
            ...
            userNotificationsRules = TNotificationsRules.query.filter(TNotificationsRules.id_role == role, TNotificationsRules.code_notification_category == category)
            ...
            for rule in userNotificationsRules.all():
                ...
                method_exists = BibNotificationsMethods.query.filter_by(code_notification_method=method).one()
                if not method_exists:
                    raise BadRequest("This method of notification in not implement yet")
            
                # get template for this method and category
                notificationTemplate = BibNotificationsTemplates.query.filter_by(notification_template_method=method, notification_template_category=category).one()
                if not notificationTemplate:
                    log.info("No template for this notification category : %s, and method : %s", category, method)
                
                log.info("Template content : %s", notificationTemplate.notification_template_content)
