...
def create_notification(scope):
    ...
    try:
        ...
        method_exists = BibNotificationsMethods.query.filter_by(label_notification_method=method).first()
        ...
        if method == "BDD": 
            ...
            unread = "UNREAD"
            status = BibNotificationsStatus.query.filter_by(label_notification_status=unread).first()
...
def count_notification(scope):
    ...
    notificationNumber = TNotifications.query.filter(TNotifications.id_role == g.current_user.id_role).count()
