...
class SynthesefauneTable extends Doctrine_Table
{
    
    public static function getInstance()
    {
        return Doctrine_Core::getTable('Synthesefaune');
    }
    ...
    private static function addFilters($params)
    {
        ...
        if (isset($params['coeur']) && $params['coeur']== 'true')
            $sql .= " AND z.id_zone = 3249";
        if ((isset($params['patrimonial']) && $params['patrimonial']!='') || (isset($params['protection_stricte']) && $params['protection_stricte']!='')) {
            if (($params['patrimonial']=='true') || ($params['protection_stricte']=='true')){
                $sql .= " AND (";
                    if(($params['patrimonial']=='true')&&($params['protection_stricte']!='true')){$sql .= "t.patrimonial = true";}
                    if(($params['patrimonial']!='true')&&($params['protection_stricte']=='true')){$sql .= "t.protection_stricte = true";}
                    if(($params['patrimonial']=='true')&&($params['protection_stricte']=='true')){$sql .= "t.patrimonial = true OR t.protection_stricte = true";}
                $sql .= ")";
            }
        }
    ...
    private static function addTSyntheseFilters($params)
    {
        ...
        if (isset($params['taxonfr']) && $params['taxonfr']!='')
            $sql .= " AND synt.id_taxon = ".$params['taxonfr'];
        if (isset($params['taxonl']) && $params['taxonl']!='')
            $sql .= " AND synt.id_taxon = ".$params['taxonl'];
        if (isset($params['idstaxons']) && $params['idstaxons']!='')
            $sql .= " AND synt.id_taxon IN (".$params['idstaxons'].")";
    ...
    { 
        ...
        if($nb_res<10000){
            ...
            if($params['start']=="no"){
                ...
                $from = " FROM (SELECT * FROM synthese.synthesefaune synt WHERE supprime = false ".$addTSyntheseFilters.") synt "; 
            }
            else{
                $from = " FROM (SELECT * FROM synthese.synthesefaune WHERE supprime = false ORDER BY dateobs DESC limit 50) synt ";
                $addfilters = '';
            }
            $dbh = Doctrine_Manager::getInstance()->getCurrentConnection()->getDbh();
            $sql = "
                SELECT DISTINCT synt.id_synthese, synt.id_source, synt.id_fiche_source, synt.code_fiche_source, synt.id_organisme, id_protocole, synt.id_lot, l.nom_lot, p.nom_programme,
                    ...
                    t.id_taxon,t.nom_latin AS taxon_latin, t.nom_francais AS taxon_francais,t.patrimonial,t.protection_stricte,
                    txr.cd_ref,
    ...
    private static function addwhere($params)
    {
        ...
        if (isset($params['observateur']) && $params['observateur']!='')
            $sql .= " AND synt.observateurs ILIKE '%".$params['observateur']."%'";
        if (isset($params['id_unite']) && $params['id_unite']!='' && $params['id_unite']==9 && isset($params['userName']) && $params['userName']!='' ){
    ...
    public static function listXlsObs($params)
    {
        ...
        if($params['start']=="no"){$from = " FROM synthese.synthesefaune synt ";}
            ...
            $sql = "
                ...
                t.nom_latin AS taxon_latin, t.nom_francais AS taxon_francais,t.patrimonial,
    ...
    public static function listShp($params,$typ)
    {
        ...
        if($params['start']=="no"){$from = " FROM synthese.synthesefaune synt ";}
            ...
            $sql .= "t.nom_latin AS taxonlatin, t.nom_francais AS taxonfr,t.patrimonial AS patri,";
