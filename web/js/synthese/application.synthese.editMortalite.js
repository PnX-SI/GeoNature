/**
 * @class application.synthese.editMortalite
...
application.synthese.editMortalite = function() {
    // private variables
       ...
       /*** SOLUTION ATOL ***/
        ...
        if(application.synthese.user.statuscode >= 2){
            var addTaxonButton = new Ext.Button({
                ...
                ,disabled: true
                ,handler: function() {
                    oldTaxon = Ext.getCmp('grid-taxons').getSelectionModel().getSelected().get('id_taxon');
                    var monStore = Ext.getCmp('combo-fiche-taxon').getStore();
                    var regId = new RegExp ("^"+oldTaxon+"$",'gi');
                    var rec = monStore.getAt(monStore.find('id_taxon',regId));
                    this.addNewIndividu();
    ...
    var createLayer = function() {
        ...
        vectorLayer.events.on({
            featureadded: function(obj) {
                ...
                if(!firstAltitudeLoad){application.synthese.editMortalite.findZ(feature);}
        ...
        Ext.getCmp('edit-fiche-form').getForm().submit({
            ...
            ,success: function(form,action) {
                Ext.getCmp('ficheSaveButton').setText('Enregistrer');
                    ...
                    toolbarInitializedOnce = false;
                    application.synthese.editMortalite.window.destroy();
            }
    ...
    return {
        ...
        ,loadFiche: function(fiche,action,cd) {
            ...
            if (action=='update') {
                ...
                this.window.setTitle('Modification d\'un pointage mortalité');
                if (fiche) {
                    id_cf = fiche;
        ...
        ,findZ : function(feature) {
            ...
            script.src = String.format('//wxs.ign.fr/{0}/alti/rest/elevation.xml?lon={1}&lat={2}&output=json&zonly=true&callback=application.synthese.editMortalite.handleIGNResponse', ign_api_key, latLonGeom.x, latLonGeom.y);
        ...
        ,handleIGNResponse : function(data) {
            ...
            application.synthese.editMortalite.setAltitude(Math.round(s[0].innerHTML));
        }
