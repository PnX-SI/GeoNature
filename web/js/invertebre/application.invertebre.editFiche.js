/**
 * @class application.invertebre.editFiche
 * Singleton to build the editFiche window
 *
 * @singleton
 */
...
application.invertebre.editFiche = function() {
    // private variables
    ...
    var getViewportSouthItem = function() {
        return {
            ...
            ,bbar: new Ext.Toolbar({           
                ...
                application.invertebre.user.nom+' ('+application.invertebre.user.status+')',
                {
                    text: 'Déconnexion'
                    ,iconCls: 'logout'
                    ,handler: function() {
                    ...
                    ,handler: function() {
                        Ext.Msg.confirm('Attention'
                            ...
                            ,function(btn) {
                                if (btn == 'yes') {
                                    resetViewport();
                                    Ext.ux.Toast.msg('Information !', 'Aucune donnée n\'a été enregistrée, le formulaire a été réinitialisé.');
                                }
    ...
    var getFormItems = function() { 
        var comboObservateurs = new Ext.ux.form.SuperBoxSelect({
            ...
            ,store: application.invertebre.storeObservateursInvAdd
            ,displayField: 'auteur'
            ,valueField: 'id_role'
            ,allowBlank: false
            ,resizable: true
            ...
            ,selectOnFocus:true
            ,mode: 'local'
            ,value: application.invertebre.user.id_utilisateur
            ,listeners:{
                afterrender :function(combo){
                    combo.setValue(application.invertebre.user.id_utilisateur);
        ...
        ,items :[comboObservateurs
            ,{
                ...
                ,{
                    ...
                    ,anchor: '40%'
                    ,allowBlank: false
                    ,blankText:'L\'heure est obligatoire'
                    ,listeners: {
                        render: function(c) {
                            Ext.QuickTips.register({
                                target: c.getEl(),
                ...
                ,items: [
                {
                    ...
                    ,store: application.invertebre.storeMilieuxInv
                ...
                // {
                    // id: 'fieldfiche-codemilieu'
                    // ,xtype:'numberfield'
                    // ,allowDecimals :false
                    // ,allowNegative: false
                    // ,fieldLabel: 'Code milieu'
                    // ,name: 'codemilieu'
                    // ,anchor: '40%'
                    // ,listeners: {
                        // render: function(c) {
                            // Ext.QuickTips.register({
                                // target: c.getEl(),
                                // text: 'Le code milieu est celui indiqué au dos des fiches invertébrés.'
                            // });
                        // }
                    // }
                // }
            ...
            // ,{
                ...
                // ,name: 'id_lot_inv'
                // ,hiddenName:'id_lot'
                // ,store: application.invertebre.storeLotsInv
                // ,valueField: "id_lot"
    ...
    var getFormTaxons = function(){
       ...
       this.addNewTaxon = function(){
        ...
        if(application.invertebre.user.statuscode >= 2){
    ...
    var createLayer = function() {
        ...
        vectorLayer.events.on({
            featureadded: function(obj) {
                ...
                // if(Ext.getCmp('edit-fiche-form').getForm().findField('monaction').getValue() =='add'){application.invertebre.editFiche.findZ();}
    ...
    var createMap = function() {
        map = application.invertebre.createMap();
    ...
    var initToolbarItems = function() {
        if (!toolbarInitializedOnce) {
            ...
            application.invertebre.utils.addSeparator(toolbar);
            ...
            toolbar.add({
                ...
                ,handler: function() {
                    ...
                    application.invertebre.editFiche.initGpsWindow();
                }
            });
            ...
            layerTreeTip = application.invertebre.createLayerWindow(map);
    ...
    var submitForm = function() {
        ...
        Ext.getCmp('edit-fiche-form').getForm().submit({
            ...
            ,success: function(form,action) {
                ...
                Ext.getCmp('edit-fiche-form').getForm().findField('id_inv').setValue(action.result.id_inv);
                id_inv = action.result.id_inv;
                Ext.getCmp('edit-fiche-form').getForm().findField('monaction').setValue('update');
                myProxyReleves.url = 'invertebre/listreleves/'+id_inv;
                Ext.getCmp('grid-taxons').getStore().reload();
                application.invertebre.editFiche.initSaveWindow(id_inv);
            }
        ...
        return new Ext.Window({
            ...
            ,buttons: [{
                ...
                ,handler: function(){
                    ...
                    Ext.ux.Toast.msg('Et voilà !', 'C\'est reparti ! Penser à commencer par le pointage sur la carte');
                }
            },{
    ...
    return {
        ...
        ,loadFiche: function(fiche,action,cd) {
            ...
            this.viewport.show();
                ...
                Ext.getCmp('grid-taxons').getSelectionModel().selectRow(0);
                Ext.ux.Toast.msg('Information !', 'Commencer par pointer l\'observation sur la carte, puis saisissez les informations à droite');
        }
