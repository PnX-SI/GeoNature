--
-- PostgreSQL database dump
--

-- Dumped from database version 9.5.7
-- Dumped by pg_dump version 9.5.7

-- Started on 2017-08-25 14:46:02 CEST

SET statement_timeout = 0;
SET lock_timeout = 0;
SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;
SET check_function_bodies = false;
SET client_min_messages = warning;
SET row_security = off;

SET search_path = ref_geographique, pg_catalog;

SET default_tablespace = '';

SET default_with_oids = false;

--
-- TOC entry 302 (class 1259 OID 111560)
-- Name: l_zonesstatut; Type: TABLE; Schema: ref_geographique; Owner: dataadmin
--

CREATE TABLE l_zonesstatut (
    id_zone integer NOT NULL,
    id_type integer NOT NULL,
    id_mnhn character varying(20),
    nomzone character varying(250),
    geom public.geometry,
    source character varying(250),
    commentaire text,
    meta_create_timestamp timestamp without time zone,
    meta_update_timestamp timestamp without time zone,
    CONSTRAINT enforce_geotype_the_geom CHECK (((public.geometrytype(geom) = 'MULTIPOLYGON'::text) OR (geom IS NULL))),
    CONSTRAINT enforce_srid_the_geom CHECK ((public.st_srid(geom) = 2154))
);



--
-- TOC entry 304 (class 1259 OID 111572)
-- Name: bib_typeszones; Type: TABLE; Schema: ref_geographique; Owner: dataadmin
--

CREATE TABLE bib_typeszones (
    id_type integer NOT NULL,
    typezone character varying(200),
    code character varying(5)
);



--
-- TOC entry 305 (class 1259 OID 111575)
-- Name: l_communes; Type: TABLE; Schema: ref_geographique; Owner: dataadmin
--

CREATE TABLE l_communes (
    geom public.geometry(MultiPolygon,2154),
    id character varying(25) NOT NULL,
    prec_plani double precision,
    nom character varying(250),
    code_insee character varying(5),
    code_siren character varying(10),
    statut character varying(20),
    canton character varying(200),
    arrondisst character varying(45),
    code_depart character varying(3),
    depart character varying(30),
    region character varying(30),
    popul integer,
    multican character varying(3),
    cc_nom character varying(250),
    cc_siren bigint,
    cc_nature character varying(5),
    cc_date_creation character varying(10),
    cc_date_effet character varying(10),
    zc boolean,
    aa boolean,
    pec boolean,
    apa boolean,
    massif character varying(50),
    insee_commune_nouvelle character varying(5)
);



CREATE TABLE l_mailles (
    code_maille character varying(50) NOT NULL,
    geom public.geometry(Polygon,2154),
    centroid public.geometry(Point,2154),
    cxmin integer,
    cxmax integer,
    cymin integer,
    cymax integer,
    code_maille_10k character varying(20),
    zc boolean
);


CREATE TABLE l_zonesstructure (
    id_zone integer NOT NULL,
    id_type integer NOT NULL,
    code character varying(20),
    nomzone character varying(250),
    geom public.geometry(MultiPolygon,2154),
    source character varying(250),
    commentaire text,
    meta_create_timestamp timestamp without time zone,
    meta_update_timestamp timestamp without time zone
);



CREATE SEQUENCE l_zonesstructure_id_zone_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;


ALTER TABLE ONLY l_zonesstructure ALTER COLUMN id_zone SET DEFAULT nextval('l_zonesstructure_id_zone_seq'::regclass);


ALTER TABLE ONLY l_communes
    ADD CONSTRAINT l_communes_pkey PRIMARY KEY (id);

ALTER TABLE ONLY l_mailles
    ADD CONSTRAINT l_mailles_pkey PRIMARY KEY (code_maille);

ALTER TABLE ONLY l_zonesstructure
    ADD CONSTRAINT l_zonesstructure_pkey PRIMARY KEY (id_zone);

ALTER TABLE ONLY l_zonesstatut
    ADD CONSTRAINT pk_l_zonesstatut PRIMARY KEY (id_zone);

ALTER TABLE ONLY bib_typeszones
    ADD CONSTRAINT pk_typeszones PRIMARY KEY (id_type);

CREATE INDEX index_l_communes_geom ON l_communes USING gist (geom);


CREATE TRIGGER trg_date_changes BEFORE INSERT OR UPDATE ON l_zonesstatut FOR EACH ROW EXECUTE PROCEDURE public.fct_trg_date_changes();


CREATE TRIGGER trg_date_changes BEFORE INSERT OR UPDATE ON l_zonesstructure FOR EACH ROW EXECUTE PROCEDURE public.fct_trg_date_changes();

ALTER TABLE ONLY l_zonesstatut
    ADD CONSTRAINT l_zonesstatut_id_type_fkey FOREIGN KEY (id_type) REFERENCES bib_typeszones(id_type) ON UPDATE CASCADE;

ALTER TABLE ONLY l_zonesstructure
    ADD CONSTRAINT l_zonesstructure_id_type_fkey FOREIGN KEY (id_type) REFERENCES bib_typeszones(id_type) ON UPDATE CASCADE;
