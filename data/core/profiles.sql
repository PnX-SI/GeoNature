...
AS WITH classified_data AS (
 ...
 unnest(
 	ARRAY[
 		ceiling(date_part('doy'::text, vsfp.date_min) / p.temporal_precision_days::double precision)* p.temporal_precision_days::integer,
 		ceiling(date_part('doy'::text, vsfp.date_max) / p.temporal_precision_days::double precision)* p.temporal_precision_days::integer
 	]
 ) as doy_min,
  unnest(
 	ARRAY[
 		ceiling(date_part('doy'::text, vsfp.date_min) / p.temporal_precision_days::double precision)* p.temporal_precision_days + p.temporal_precision_days::integer,
 		ceiling(date_part('doy'::text, vsfp.date_max) / p.temporal_precision_days::double precision)* p.temporal_precision_days + p.temporal_precision_days::integer
 	]
 ) doy_max,                
 CASE
 ...
 WHERE p.temporal_precision_days IS NOT NULL 
       AND p.spatial_precision IS NOT NULL AND p.active_life_stage IS NOT NULL 
