...
def insert_update_t_acquisition_frameworks(CURRENT_AF_ROOT, action, cur_af_uuid):
	...
	motCle='\''+str(get_tuple_data(CURRENT_AF_ROOT, af_main, 'motCle')).replace("'","").replace("[","").replace("]","").replace('"','')+'\''
	descriptionCible=get_single_data(CURRENT_AF_ROOT, af_main,'descriptionCible')
	cibleEcologiqueOuGeologique=get_single_data(CURRENT_AF_ROOT, af_main,'cibleEcologiqueOuGeologique')
	...
	if get_single_data(CURRENT_AF_ROOT, af_main,'niveauTerritorial')=='\'\'':
		...
		id_niveauTerritorial='(SELECT n.id_nomenclature FROM ref_nomenclatures.t_nomenclatures n,' \
			+'ref_nomenclatures.bib_nomenclatures_types t WHERE t.id_type=n.id_type AND t.mnemonique=\'NIVEAU_TERRITORIAL\' AND' \
			+' cd_nomenclature='+get_single_data(CURRENT_AF_ROOT, af_main,'niveauTerritorial')+')::integer'
	...
	if get_single_data(CURRENT_AF_ROOT, af_main,'typeFinancement')=='\'\'':
		...
		id_typeFinancement='(SELECT n.id_nomenclature FROM ref_nomenclatures.t_nomenclatures n, ref_nomenclatures.bib_nomenclatures_types t  WHERE' \
			+' t.id_type=n.id_type AND t.mnemonique=\'TYPE_FINANCEMENT\' AND cd_nomenclature='+get_single_data(CURRENT_AF_ROOT, af_main,'typeFinancement')+')::integer'
	...
	if get_single_data(CURRENT_AF_ROOT, af_temp_ref,'dateCloture')=='\'\'':
		...
		dateCloture=get_single_data(CURRENT_AF_ROOT, af_temp_ref,'dateCloture')+'::timestamp without time zone'
	# dateCreationMtd
...
def insert_update_t_datasets(CURRENT_DS_ROOT, action,cur_ds_uuid, id_ca):
	...
	if get_single_data(CURRENT_DS_ROOT,ds_main, 'description')!='':
		...
		dataset_desc = '\'\''
	keywords='\''+str(get_tuple_data(CURRENT_DS_ROOT, ds_main, 'motCle')).replace("'","").replace("[","").replace("]","").replace('"','')+'\''
	...
	if get_single_data(CURRENT_DS_ROOT, ds_main,'dateCreation')=="''":
		...
		meta_create_date=get_single_data(CURRENT_DS_ROOT, ds_main,'dateCreation')+'::timestamp without time zone'
	# dateMiseAJourMtd
	if get_single_data(CURRENT_DS_ROOT,ds_main, 'dateRevision')=="''" :
		meta_update_date='NULL'
	else :
		meta_update_date=get_single_data(CURRENT_DS_ROOT, ds_main,'dateRevision')+'::timestamp without time zone'
	# If several objectives, set default value 'Autre'
	if len(get_tuple_data(CURRENT_DS_ROOT, ds_main,'objectifJdd')) == 1 :
		id_nomenclature_dataset_objectif = 'ref_nomenclatures.get_id_nomenclature(\'JDD_OBJECTIFS\', '+get_single_data(CURRENT_DS_ROOT, ds_main,'objectifJdd')+')'
	else :	
	...
	if action=='create':
		cur_query='INSERT INTO gn_meta.t_datasets(unique_dataset_id, id_acquisition_framework, dataset_name, dataset_shortname, '\
			...
			+id_nomenclature_source_status+', '+id_nomenclature_resource_type+', '+validable+', '+active+','+meta_create_date+', '+meta_update_date+' );'
...
def insert_CA(cur_af_uuid):
	...
	if request.status_code == 200:
		...
		af_publications=CURRENT_AF_ROOT.findall('gml:featureMember/ca:CadreAcquisition/ca:referenceBiblio/ca:Publication',xml_namespaces)
		...
		if get_single_data(CURRENT_AF_ROOT, af_main_actor, 'nomPrenom') != "''" :
			cur_person_name = get_single_data(CURRENT_AF_ROOT, af_main_actor, 'nomPrenom').replace('\t','').replace("'","").rstrip()
		...
		if get_single_data(CURRENT_AF_ROOT, af_main_actor, 'idOrganisme') != "''" and get_single_data(CURRENT_AF_ROOT ,af_main_actor, 'organisme') != "''" :
		...
		# For others actors
		af_othersactors = CURRENT_AF_ROOT.findall('gml:featureMember/ca:CadreAcquisition/ca:acteurAutre/ca:ActeurType',xml_namespaces)
