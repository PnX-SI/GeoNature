...
def insert_person(cur_person_name, cur_person_mail):
    if len(cur_person_name.rsplit(" ", 1)) == 2:
        ...
        role_firstname = ""
    create_role = """
	INSERT INTO utilisateurs.t_roles (nom_role, prenom_role, email) 
	VALUES (
		'{role_name}', 
		'{role_firstname}',
		'{cur_person_mail}'
		...
		""".format(
        role_name=role_name,
        role_firstname=role_firstname,
        cur_person_mail=cur_person_mail.replace("'", ""),
    )
...
def update_person(cur_person_name, cur_person_mail):
    ...
    update_role = """
		...
		SET 
			nom_role='{role_name}',
		  prenom_role='{role_firstname}',
		  email='{cur_person_mail}', 
		WHERE nom_role||(
			CASE WHEN prenom_role=\'\' THEN \'\' ELSE \' \'||prenom_role END)
			= '{cur_person_name}'
		""".format(
        role_name=role_name,
        role_firstname=role_firstname,
        cur_person_mail=cur_person_mail.replace("'", ""),
        cur_person_name=cur_person_name,
    )
...
def insert_update_t_acquisition_frameworks(CURRENT_AF_ROOT, action, cur_af_uuid):
    ...
    if action == "create":
		...
		""".format(
            ...
            motCle=motCle,
            id_typeFinancement=id_typeFinancement,
            descriptionCible=descriptionCible,
            cibleEcologiqueOuGeologique=cibleEcologiqueOuGeologique,
            estMetaCadre=estMetaCadre,
...
def delete_cor_af(table, cur_af_uuid):
	...
	""".format(
        table=table, cur_af_uuid=cur_af_uuid
    )
        ...
        for cur_proto in range(len(ds_protocols)):
            if get_inner_data(ds_protocols, cur_proto, "libelleProtocole") != "''":
                ...
                if get_inner_data(ds_protocols, cur_proto, "descriptionProtocole") != None:
                    cur_protocol_desc = get_inner_data(
                        ds_protocols, cur_proto, "descriptionProtocole"
                    )
                else:
                    cur_protocol_desc = "''"
