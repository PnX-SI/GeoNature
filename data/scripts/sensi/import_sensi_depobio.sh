...
COPY gn_sensitivity.tmp_rules FROM '/tmp/ref_sensibilite_21122020.csv' DELIMITER ';' CSV HEADER;
"
...
cd_nom , nom_cite ,  id_nomenclature_sensitivity,  sensitivity_duration , sensitivity_territory, id_territory , date_min , date_max , "source" , active , "comments"
)
select t.cd_ref_v13, t.nom_cite , ref_nomenclatures.get_id_nomenclature('SENSIBILITE', codage::varchar)  , coalesce(t.duree, 10000) , 'Département', t.cd_dept, null, null, 'Fichier Solene issu BDC', true, null
from gn_sensitivity.tmp_rules t;


-- insertion dans cor_area_sensi
