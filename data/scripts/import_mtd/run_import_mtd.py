...
def get_single_data(path, tag):
    ...
    try:
        data = (
            root.find(path + tag, namespaces=xml_namespaces)
            .text.replace("'", "''")
            .replace("’", "''")
            .replace('"', "")
            .replace("\u202f", " ")
...
def insert_update_t_acquisition_frameworks(action, cur_af_uuid):
    ...
    motCle = (
        "'"
        + str(get_tuple_data(af_main, "motCle"))
        .replace("'", "")
        .replace("[", "")
        .replace("]", "")
        .replace('"', "")
    ...
    if get_single_data(af_main, "niveauTerritorial") == "''":
        ...
        id_niveauTerritorial = (
            ...
            + "ref_nomenclatures.bib_nomenclatures_types t WHERE t.id_type=n.id_type AND t.mnemonique='NIVEAU_TERRITORIAL' AND"
            + " cd_nomenclature="
            + get_single_data(af_main, "niveauTerritorial")
            + ")::integer"
        )
    ...
    if get_single_data(af_main, "typeFinancement") == "''":
        ...
        id_typeFinancement = (
            ...
            + get_single_data(af_main, "typeFinancement")
            + ")::integer"
        )
    ...
    if get_single_data(af_temp_ref, "dateLancement") == "''":
        ...
        dateLancement = (
            get_single_data(af_temp_ref, "dateLancement") + "::timestamp without time zone"
        )
    # dateCloture
    ...
    if get_single_data(af_main, "dateCreationMtd") == "''":
        ...
        dateCreationMtd = (
            get_single_data(af_main, "dateCreationMtd") + "::timestamp without time zone"
        )
    # dateMiseAJourMtd
    if get_single_data(af_main, "dateMiseAJourMtd") == "''":
        ...
        dateMiseAJourMtd = (
            get_single_data(af_main, "dateMiseAJourMtd") + "::timestamp without time zone"
        )
    # Write and run query
    if action == "create":
        cur_query = (
            "INSERT INTO gn_meta.t_acquisition_frameworks(unique_acquisition_framework_id, acquisition_framework_name, "
        ...
        cur_query = (
            "UPDATE gn_meta.t_acquisition_frameworks SET acquisition_framework_name="
            + libelle
            ...
            + ", meta_update_date="
            + dateMiseAJourMtd
            + " WHERE unique_acquisition_framework_id='"
            + cur_af_uuid
            + "'"
        )
...
def delete_cor_af(table, cur_af_uuid):
    ...
    cur_delete_query = (
        "DELETE FROM gn_meta.cor_acquisition_framework_"
        + table
        + " WHERE id_acquisition_framework=(SELECT id_acquisition_framework FROM gn_meta.t_acquisition_frameworks WHERE unique_acquisition_framework_id='"
        + cur_af_uuid
        + "')"
    )
...
def insert_update_t_datasets(action, cur_ds_uuid):
    ...
    id_acquisition_framework = (
        "(SELECT id_acquisition_framework FROM gn_meta.t_acquisition_frameworks WHERE unique_acquisition_framework_id="
    ...
    keywords = (
        "'"
        + str(get_tuple_data(ds_main, "motCle"))
        .replace("'", "")
        .replace("[", "")
        .replace("]", "")
        .replace('"', "")
    ...
    if get_single_data(ds_main, "dateCreation") == "''":
        ...
        meta_create_date = (
            get_single_data(ds_main, "dateCreation") + "::timestamp without time zone"
        )
    # dateMiseAJourMtd
    if get_single_data(ds_main, "dateRevision") == "''":
        ...
        meta_update_date = (
            get_single_data(ds_main, "dateRevision") + "::timestamp without time zone"
        )
    ...
    if len(get_tuple_data(ds_main, "objectifJdd")) == 1:
        id_nomenclature_dataset_objectif = (
            ...
            + get_single_data(ds_main, "objectifJdd")
            + ")"
        )
    else:
        id_nomenclature_dataset_objectif = (
...
for af_iter in range(len(af_uuid_list)):
    ...
    af_URL = "https://inpn.mnhn.fr/mtd/cadre/export/xml/GetRecordById?id={}".format(
        cur_af_uuid.upper()
    )
    ...
    af_publications = root.findall(
        "gml:featureMember/ca:CadreAcquisition/ca:referenceBiblio/ca:Publication", xml_namespaces
    )
    ...
    if get_single_data(af_main_actor, "nomPrenom") != "''":
        cur_person_name = (
            get_single_data(af_main_actor, "nomPrenom").replace("\t", "").replace("'", "").rstrip()
        )
    ...
    if (
        get_single_data(af_main_actor, "idOrganisme") != "''"
        and get_single_data(af_main_actor, "organisme") != "''"
    ):
...
for ds_iter in range(len(ds_uuid_list)):
    ...
    # For contact_points
    ds_pointscontacts = root.findall(
    ...
    if get_single_data(ds_contact_pf, "nomPrenom") != "''":
        cur_person_name = (
            get_single_data(ds_contact_pf, "nomPrenom").replace("\t", "").replace("'", "").rstrip()
        )
    ...
    # Organism : the uuid is the reference
    if (
        get_single_data(ds_contact_pf, "idOrganisme") != "''"
        and get_single_data(ds_contact_pf, "organisme") != "''"
    ):
