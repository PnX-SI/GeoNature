...
def get_single_data(node, path, tag):
    ...
    try:
        data = (
            node.find(path + tag, namespaces=xml_namespaces)
            .text.replace("'", "''")
            .replace("’", "''")
            .replace('"', "")
            .replace("\u202f", " ")
...
def insert_sinp_datatype_protocols(cur_protocol_name, cur_protocol_desc, cur_protocol_url):
    ...
    create_protocol = (
        ...
        + "', '"
        + cur_protocol_desc
        + "', SELECT(ref_nomenclatures.get_id_nomenclature('TYPE_PROTOCOLE', '0')), '"
        + cur_protocol_url
        + "')"
    )
...
def update_sinp_datatype_protocols(cur_protocol_name, cur_protocol_desc, cur_protocol_url):
    # Protocol type not found in XML files, by default 'inconnu'
    update_protocol = (
        "UPDATE gn_meta.sinp_datatype_protocols SET protocol_desc='"
        + cur_protocol_desc
        + "', "
        + "id_nomenclature_protocol_type=ref_nomenclatures.get_id_nomenclature('TYPE_PROTOCOLE', '0'), protocol_url='"
        + cur_protocol_url
        + "' WHERE protocol_name='"
        + cur_protocol_name
        + "'"
...
def insert_sinp_datatype_publications(cur_publication, cur_url):
    create_publication = (
        "INSERT INTO gn_meta.sinp_datatype_publications (publication_reference,publication_url)"
        + " VALUES ("
        + cur_publication
        + ", "
        + cur_url
...
def update_sinp_datatype_publications(cur_publication, cur_url):
    update_publication = (
        "UPDATE gn_meta.sinp_datatype_publications SET publication_url="
        + cur_url
        + " WHERE publication_reference="
        + cur_publication
    )
...
def insert_organism(cur_organism_uuid, cur_organism_name):
    create_organism = (
        "INSERT INTO utilisateurs.bib_organismes (uuid_organisme,nom_organisme) VALUES ("
        + cur_organism_uuid
        + ", "
        + cur_organism_name
        + ")"
...
def update_organism(cur_organism_uuid, cur_organism_name):
    update_organism = (
        "UPDATE utilisateurs.bib_organismes SET uuid_organisme="
        + cur_organism_uuid
        + ", nom_organisme="
        + cur_organism_name
        + " WHERE uuid_organisme="
        + str.lower(cur_organism_uuid)
    )
...
def get_known_persons():
    cursor.execute(
        "SELECT DISTINCT nom_role||(CASE WHEN prenom_role='' THEN '' ELSE ' '||prenom_role END) FROM utilisateurs.t_roles WHERE groupe='False'"
    )
...
def insert_person(cur_person_name, cur_person_mail):
    if len(cur_person_name.rsplit(" ", 1)) == 2:
        ...
        role_firstname = ""
    create_role = (
        "INSERT INTO utilisateurs.t_roles (nom_role, prenom_role, email, id_organisme) VALUES ('"
        + role_name
        + "', '"
        + role_firstname
        + "', '"
        + cur_person_mail.replace("'", "")
        + "', '"
        + str(DEFAULT_ID_ORGANISME)
        + "')"
    )
...
def update_person(cur_person_name, cur_person_mail):
    if len(cur_person_name.rsplit(" ", 1)) == 2:
        ...
        role_firstname = ""
    update_role = (
        "UPDATE utilisateurs.t_roles SET nom_role='"
        + role_name
        + "', prenom_role='"
        + role_firstname
        + "', email='"
        ...
        + str(DEFAULT_ID_ORGANISME)
        + "' "
        + "WHERE nom_role||(CASE WHEN prenom_role='' THEN '' ELSE ' '||prenom_role END)='"
        + cur_person_name
        + "'"
    )
...
def insert_update_t_acquisition_frameworks(CURRENT_AF_ROOT, action, cur_af_uuid):
    ...
    motCle = (
        "'"
        + str(get_tuple_data(CURRENT_AF_ROOT, af_main, "motCle"))
        .replace("'", "")
        .replace("[", "")
        .replace("]", "")
        .replace('"', "")
    ...
    descriptionCible = get_single_data(CURRENT_AF_ROOT, af_main, "descriptionCible")
    cibleEcologiqueOuGeologique = get_single_data(
        CURRENT_AF_ROOT, af_main, "cibleEcologiqueOuGeologique"
    )
    precisionGeographique = get_single_data(CURRENT_AF_ROOT, af_main, "precisionGeographique")
    #
    # territorial level : DEFAULT='National'
    if get_single_data(CURRENT_AF_ROOT, af_main, "niveauTerritorial") == "''":
        id_niveauTerritorial = (
            "(SELECT n.id_nomenclature FROM ref_nomenclatures.t_nomenclatures n,"
        ...
        id_niveauTerritorial = (
            ...
            + "ref_nomenclatures.bib_nomenclatures_types t WHERE t.id_type=n.id_type AND t.mnemonique='NIVEAU_TERRITORIAL' AND"
            + " cd_nomenclature="
            + get_single_data(CURRENT_AF_ROOT, af_main, "niveauTerritorial")
            + ")::integer"
        )
    # Financing Type : DEFAULT="Publique"
    if get_single_data(CURRENT_AF_ROOT, af_main, "typeFinancement") == "''":
        id_typeFinancement = (
            "(SELECT n.id_nomenclature FROM ref_nomenclatures.t_nomenclatures n, ref_nomenclatures.bib_nomenclatures_types t  WHERE"
            + " t.id_type=n.id_type AND t.mnemonique='TYPE_FINANCEMENT' AND cd_nomenclature='1')::integer"
        )
    else:
        id_typeFinancement = (
            "(SELECT n.id_nomenclature FROM ref_nomenclatures.t_nomenclatures n, ref_nomenclatures.bib_nomenclatures_types t  WHERE"
            + " t.id_type=n.id_type AND t.mnemonique='TYPE_FINANCEMENT' AND cd_nomenclature="
            + get_single_data(CURRENT_AF_ROOT, af_main, "typeFinancement")
            + ")::integer"
        )
    # estMetaCadre : DEFAULT=False
    if get_single_data(CURRENT_AF_ROOT, af_main, "estMetaCadre") == "''":
        estMetaCadre = "false"
    else:
        estMetaCadre = get_single_data(CURRENT_AF_ROOT, af_main, "estMetaCadre")
    ...
    if get_single_data(CURRENT_AF_ROOT, af_temp_ref, "dateLancement") == "''":
        ...
        dateLancement = (
            get_single_data(CURRENT_AF_ROOT, af_temp_ref, "dateLancement")
    ...
    if get_single_data(CURRENT_AF_ROOT, af_temp_ref, "dateCloture") == "''":
        ...
        dateCloture = (
            get_single_data(CURRENT_AF_ROOT, af_temp_ref, "dateCloture")
            + "::timestamp without time zone"
        )
    # dateCreationMtd
    if get_single_data(CURRENT_AF_ROOT, af_main, "dateCreationMtd") == "''":
        ...
        dateCreationMtd = (
            get_single_data(CURRENT_AF_ROOT, af_main, "dateCreationMtd")
            + "::timestamp without time zone"
        )
    # dateMiseAJourMtd
    if get_single_data(CURRENT_AF_ROOT, af_main, "dateMiseAJourMtd") == "''":
        ...
        dateMiseAJourMtd = (
            get_single_data(CURRENT_AF_ROOT, af_main, "dateMiseAJourMtd")
            + "::timestamp without time zone"
        )
    # Write and run query
    if action == "create":
        ...
        cur_query = f"""
			UPDATE gn_meta.t_acquisition_frameworks SET 
				...
				id_nomenclature_territorial_level={id_niveauTerritorial}, 
				territory_desc={precisionGeographique}, 
				keywords={motCle},
				id_nomenclature_financing_type={id_typeFinancement}, 
				target_description={descriptionCible}, 
				ecologic_or_geologic_target={cibleEcologiqueOuGeologique},
				is_parent={estMetaCadre}, 
...
def delete_cor_af(table, cur_af_uuid):
    ...
    cur_delete_query = f"""
	DELETE FROM gn_meta.cor_acquisition_framework_{table} 
	WHERE id_acquisition_framework=(
		SELECT id_acquisition_framework FROM gn_meta.t_acquisition_frameworks 
		WHERE unique_acquisition_framework_id='{cur_af_uuid}'
	)
	"""
...
def insert_cor_af_voletsinp(cur_af_uuid, cur_volet_sinp):
    cur_insert_query = (
        ...
        + "VALUES ((SELECT id_acquisition_framework FROM gn_meta.t_acquisition_frameworks WHERE unique_acquisition_framework_id='"
        + cur_af_uuid
        + "'), "
        + "(SELECT id_nomenclature FROM ref_nomenclatures.t_nomenclatures WHERE id_type='113' AND cd_nomenclature='"
        + cur_volet_sinp
        + "'))"
    )
...
def insert_cor_af_objectifs(cur_af_uuid, cur_objectif):
    cur_insert_query = (
        ...
        + cur_af_uuid
        + "'), "
        + "(SELECT id_nomenclature FROM ref_nomenclatures.t_nomenclatures WHERE id_type='108' AND cd_nomenclature='"
        + cur_objectif
        + "'))"
    )
    cursor.execute(cur_insert_query)
...
def insert_cor_af_territory(cur_af_uuid, cur_territory):
    cur_insert_query = (
        ...
        + cur_af_uuid
        + "'), "
        + "ref_nomenclatures.get_id_nomenclature('TERRITOIRE', '"
        + cur_territory
        + "'))"
    )
    cursor.execute(cur_insert_query)
...
def insert_cor_af_publications(cur_af_uuid, af_publications):
    cur_insert_query = (
        "INSERT INTO gn_meta.cor_acquisition_framework_publication (id_acquisition_framework,id_publication)"
        + "VALUES ((SELECT id_acquisition_framework FROM gn_meta.t_acquisition_frameworks WHERE unique_acquisition_framework_id='"
        + cur_af_uuid
        + "'), "
        + "(SELECT id_publication FROM gn_meta.sinp_datatype_publications WHERE publication_reference="
        + cur_publication
        + "))"
    )
    cursor.execute(cur_insert_query)
...
def insert_cor_af_actor_organism(cur_af_uuid, cur_organism_uuid, cur_actor_role):
    cur_insert_query = (
        ...
        + cur_af_uuid
        + "'), "
        + "(SELECT id_organisme FROM utilisateurs.bib_organismes WHERE uuid_organisme="
        + str.lower(cur_organism_uuid)
        + "), "
...
def insert_cor_af_actor_person(cur_af_uuid, cur_person_name, cur_actor_role):
    cur_insert_query = (
        ...
        + cur_af_uuid
        + "'), "
        + "(SELECT id_role FROM utilisateurs.t_roles WHERE nom_role||(CASE WHEN prenom_role='' THEN '' ELSE ' '||prenom_role END)='"
        + cur_person_name
        + "'), "
        + "ref_nomenclatures.get_id_nomenclature('ROLE_ACTEUR', "
        + cur_actor_role
...
def insert_update_t_datasets(CURRENT_DS_ROOT, action, cur_ds_uuid, id_ca):
    # id_acquisition_framework = '(SELECT id_acquisition_frameget_tuple_datawork FROM gn_meta.t_acquisition_frameworks WHERE unique_acquisition_framework_id='\
    # 	+ str.lower(get_single_data(CURRENT_DS_ROOT, af_main(ds_main,'identifiantCadre'))+')'
    id_acquisition_framework = id_ca
    dataset_name = get_single_data(CURRENT_DS_ROOT, ds_main, "libelle")
    dataset_shortname = get_single_data(CURRENT_DS_ROOT, ds_main, "libelleCourt")
    if get_single_data(CURRENT_DS_ROOT, ds_main, "description") != "":
        dataset_desc = get_single_data(CURRENT_DS_ROOT, ds_main, "description")
    else:
        dataset_desc = "''"
    keywords = (
        "'"
        + str(get_tuple_data(CURRENT_DS_ROOT, ds_main, "motCle"))
        .replace("'", "")
        .replace("[", "")
        .replace("]", "")
        .replace('"', "")
    ...
    marine_domain = get_single_data(CURRENT_DS_ROOT, ds_main, "domaineMarin")
    terrestrial_domain = get_single_data(CURRENT_DS_ROOT, ds_main, "domaineTerrestre")
    validable = "true"
    bbox_west = get_single_data(CURRENT_DS_ROOT, ds_bbox, "borneOuest")
    bbox_east = get_single_data(CURRENT_DS_ROOT, ds_bbox, "borneEst")
    bbox_south = get_single_data(CURRENT_DS_ROOT, ds_bbox, "borneSud")
    bbox_north = get_single_data(CURRENT_DS_ROOT, ds_bbox, "borneNord")
    ...
    active = "false"
    # dateCreationMtd
    if get_single_data(CURRENT_DS_ROOT, ds_main, "dateCreation") == "''":
        meta_create_date = "NULL"
    else:
        meta_create_date = (
            get_single_data(CURRENT_DS_ROOT, ds_main, "dateCreation")
            + "::timestamp without time zone"
        )
    # dateMiseAJourMtd
    if get_single_data(CURRENT_DS_ROOT, ds_main, "dateRevision") == "''":
        meta_update_date = "NULL"
    else:
        meta_update_date = (
            get_single_data(CURRENT_DS_ROOT, ds_main, "dateRevision")
            + "::timestamp without time zone"
        )
    # If several objectives, set default value 'Autre'
    if len(get_tuple_data(CURRENT_DS_ROOT, ds_main, "objectifJdd")) == 1:
        id_nomenclature_dataset_objectif = (
            "ref_nomenclatures.get_id_nomenclature('JDD_OBJECTIFS', "
            + get_single_data(CURRENT_DS_ROOT, ds_main, "objectifJdd")
            + ")"
        )
    else:
        id_nomenclature_dataset_objectif = (
            "ref_nomenclatures.get_id_nomenclature('JDD_OBJECTIFS', '7.1')"
        )
    # If action is CREATE
    if action == "create":
        cur_query = (
        ...
        cur_query = f"""
			...
			id_nomenclature_dataset_objectif={id_nomenclature_dataset_objectif},
			bbox_west={bbox_west},
			bbox_east={bbox_east},
			bbox_south={bbox_south},
			bbox_north={bbox_north},
			id_nomenclature_collecting_method={id_nomenclature_collecting_method},
			id_nomenclature_data_origin={id_nomenclature_data_origin},
			id_nomenclature_source_status={id_nomenclature_source_status},
			id_nomenclature_resource_type={id_nomenclature_resource_type},
			validable={validable},
			active={active},
...
def delete_cor_ds(table, cur_ds_uuid):
    ...
    cur_delete_query = (
        "DELETE FROM gn_meta.cor_dataset_"
        + table
        + " WHERE id_dataset=(SELECT id_dataset FROM gn_meta.t_datasets WHERE unique_dataset_id='"
        + cur_ds_uuid
        + "')"
    )
    cursor.execute(cur_delete_query)
...
def insert_cor_ds_territory(cur_ds_uuid, cur_territory):
    cur_insert_query = (
        ...
        + "VALUES ((SELECT id_dataset FROM gn_meta.t_datasets WHERE unique_dataset_id='"
        + cur_ds_uuid
        + "'), "
        + "ref_nomenclatures.get_id_nomenclature('TERRITOIRE', '"
        + cur_territory
...
def insert_cor_ds_protocol(cur_ds_uuid, cur_protocol):
    cur_insert_query = (
        ...
        + cur_ds_uuid
        + "'), "
        + "(SELECT id_protocol FROM gn_meta.sinp_datatype_protocols WHERE protocol_name='"
        + cur_protocol_name
        + "'))"
    )
    cursor.execute(cur_insert_query)
...
def insert_CA(cur_af_uuid):
    ...
    if request.status_code == 200:
        ...
        volets_sinp = get_tuple_data(CURRENT_AF_ROOT, af_main, "voletSINP")
        if volets_sinp != "''":
        ...
        af_objectifs = get_tuple_data(CURRENT_AF_ROOT, af_main, "objectif")
        if af_objectifs != "''":
        ...
        cursor.execute(
            "select exists(select * from information_schema.tables where table_name='cor_acquisition_framework_territory')"
        )
        if cursor.fetchone()[0] == True:
            delete_cor_af("territory", cur_af_uuid)
            af_territories = get_tuple_data(CURRENT_AF_ROOT, af_main, "territoire")
            if af_territories != "''":
        ...
        af_publications = CURRENT_AF_ROOT.findall(
            "gml:featureMember/ca:CadreAcquisition/ca:referenceBiblio/ca:Publication",
            xml_namespaces,
        )
        ...
        cur_actor_role = get_single_data(CURRENT_AF_ROOT, af_main_actor, "roleActeur")
        # Person : name is the reference
        if get_single_data(CURRENT_AF_ROOT, af_main_actor, "nomPrenom") != "''":
            cur_person_name = (
                get_single_data(CURRENT_AF_ROOT, af_main_actor, "nomPrenom")
                .replace("\t", "")
                .replace("'", "")
                .rstrip()
            )
            if get_single_data(CURRENT_AF_ROOT, af_main_actor, "mail") != "''":
                cur_person_mail = get_single_data(CURRENT_AF_ROOT, af_main_actor, "mail").replace(
                    "'", ""
                )
            else:
                cur_person_mail = ""
        ...
        if (
            get_single_data(CURRENT_AF_ROOT, af_main_actor, "idOrganisme") != "''"
            and get_single_data(CURRENT_AF_ROOT, af_main_actor, "organisme") != "''"
        ):
            cur_organism_uuid = get_single_data(CURRENT_AF_ROOT, af_main_actor, "idOrganisme")
            cur_organism_name = get_single_data(CURRENT_AF_ROOT, af_main_actor, "organisme")
        ...
        # For others actors
        af_othersactors = CURRENT_AF_ROOT.findall(
            "gml:featureMember/ca:CadreAcquisition/ca:acteurAutre/ca:ActeurType", xml_namespaces
        )
        for other_actor in range(len(af_othersactors)):
            ...
            if get_inner_data(af_othersactors, other_actor, "nomPrenom") != "''":
                cur_person_name = (
                    get_inner_data(af_othersactors, other_actor, "nomPrenom")
                    .replace("\t", "")
                    .replace("'", "")
                    .rstrip()
                )
            ...
            # Organism : the uuid is the reference
            if (
                get_inner_data(af_othersactors, other_actor, "idOrganisme") != "''"
                and get_inner_data(af_othersactors, other_actor, "organisme") != "''"
            ):
...
for ds_iter in range(len(ds_uuid_list)):
    ...
    ds_protocols = CURRENT_DS_ROOT.findall(
        "gml:featureMember/jdd:JeuDeDonnees/jdd:protocoles/jdd:ProtocoleType", xml_namespaces
    )
    ...
    # For contact_points
    ds_pointscontacts = CURRENT_DS_ROOT.findall(
        "gml:featureMember/jdd:JeuDeDonnees/jdd:pointContactJdd/jdd:ActeurType", xml_namespaces
    )
    for point_contact in range(len(ds_pointscontacts)):
        ...
        if get_inner_data(ds_pointscontacts, point_contact, "nomPrenom") != "''":
            cur_person_name = (
                get_inner_data(ds_pointscontacts, point_contact, "nomPrenom")
                .replace("\t", "")
                .replace("'", "")
                .rstrip()
            )
        ...
        # Organism : the uuid is the reference
        if (
            get_inner_data(ds_pointscontacts, point_contact, "idOrganisme") != "''"
            and get_inner_data(ds_pointscontacts, point_contact, "organisme") != "''"
        ):
    ...
    if get_single_data(CURRENT_DS_ROOT, ds_contact_pf, "nomPrenom") != "''":
        cur_person_name = (
            get_single_data(CURRENT_DS_ROOT, ds_contact_pf, "nomPrenom")
            .replace("\t", "")
            .replace("'", "")
            .rstrip()
        )
    ...
    # Organism : the uuid is the reference
    if (
        get_single_data(CURRENT_DS_ROOT, ds_contact_pf, "idOrganisme") != "''"
        and get_single_data(CURRENT_DS_ROOT, ds_contact_pf, "organisme") != "''"
    ):
