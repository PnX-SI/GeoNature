...
DECLARE
    ...
    occurrence RECORD;
BEGIN
    --SUPRESSION EN SYNTHESE A PARTIR DE L'UUID
    DELETE FROM gn_synthese.synthese WHERE unique_id_sinp = OLD.unique_id_sinp_occtax;

    -- SUPPRESSION DE L'OCCURRENCE S'IL N'Y A PLUS DE DENOMBREMENT
    ...
    -- SUPPRESSION DU RELEVE S'IL N'A PLUS D'OCCURRENCE
    SELECT INTO occurrence occ.id_releve_occtax, count(rel.*) as nb_occ
    FROM pr_occtax.t_occurrences_occtax occ
    JOIN pr_occtax.t_releves_occtax rel ON rel.id_releve_occtax = occ.id_releve_occtax
    WHERE id_occurrence_occtax = OLD.id_occurrence_occtax
    GROUP BY occ.id_releve_occtax;
      IF occurrence.nb_occ < 1 THEN
        DELETE FROM pr_occtax.t_releves_occtax WHERE id_occurrence_occtax = occurrence.id_releve_occtax;
    END IF;
    RETURN NULL; 
END;
$BODY$
