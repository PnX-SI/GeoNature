...
- [Import] Ajout de la priorisation de la géométrie utilisée lors des imports. L'ordre de priorisation des géométries est le suivant : WKT > Long/Lat > CodeMaille > CodeCommune > CodeDepartement (#4099 par @christophe-ramet)
- [Accueil] Affichage de l'organisme de l'utilisateur authentifié dans la barre de navigation latérale, désactivable avec le nouveau paramètre `DISPLAY_USER_ORGANISM` (#4081 par @edelclaux)
- [Admin des permissions] Ajout du code de département dans le formulaire de sélection de filtres géographiques (#4084 par @jacquesfize)
...
- [Métadonnées] Correction de l'affichage du nombre de jeux de données dans l'export PDF des CA (#3985 par @jacquesfize)
...
- [Synthèse] Correction de la récupération des zonages d'une observation quand un utilisateur possède une permission floutée et une permission avec un filtre géographique (#4065 par @jacquesfize)
...
- [Synthèse] Correction des fixtures `synthese` pour qu'elles s'appuient sur le SRID de la base de données de test (#4113 par @bouttier)
- [Documentation] Documentation de l'arborescence des fichiers du backend (#4140 par @CynthiaBorotPNV)
